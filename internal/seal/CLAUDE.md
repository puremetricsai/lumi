...
- **`Key`'s zero value is a working pass-through, and that is why this is a type.** Every caller —
  capture and the backfill — is written once and behaves correctly whether or not
...
- **`TempCopy` writes to `$TMPDIR`, never beside the media.** `internal/compress`'s reconcile walk
  ...
  waiting for the next `prune --all`. It exists at all because SpeechAnalyzer and the audio decoders take paths
  rather than readers.
...
- **`SealFile` replaces through a sibling scratch file, flushed before the rename.** It is overwriting
  the only copy of a captured file. The directory is not flushed per file: a lost rename leaves the
  original beside a complete scratch file, which the next attempt removes. Callers whose next step
  depends on the rename call `SyncDir` once.
