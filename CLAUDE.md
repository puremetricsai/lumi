...
- **`ask` derives its time window from the question.** `parseTimeWindow` (`internal/cli/whenparse.go`) maps a recognized natural-time expression ("around 9:15 pm", "yesterday", "this morning", "last 2 hours") onto `Since`/`Until` in the user's local timezone, strips the expression so its words never leak into the FTS terms, and prints the interpreted window to stderr. An explicit `--since` flag skips derivation entirely. Rendering (`contextFor`) and interpretation are both local-time; only storage and range comparisons stay UTC.
