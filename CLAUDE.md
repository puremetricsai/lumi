...
task app | task app:install | task app:run  # build, install (~/Applications), launch Lumi.app
./scripts/restart-lumi-app.sh               # the app dev loop: quit, rebuild, reset TCC, relaunch
```
...
| `internal/seal` | Pure: one file format (`LUMIENC1` ‖ nonce ‖ AES-256-GCM) and the operations Lumi performs on it. Its zero-value key is a pass-through, so callers have no encrypted variant. |
...
- **Encryption is one toggle with three rules that no package may restate.** `lumi encrypt` seals every
  ...
  - **The database file decides whether a store is encrypted; the Keychain only supplies the key.** The
    one exception is a store with no database yet, where a key means the next database is created
    encrypted. `encrypt status` makes every judgement — on, incomplete, unrecoverable — and the app
    displays them. → `internal/cli/CLAUDE.md`, `macos/CLAUDE.md`
  ...
  - **While encryption is on, `lumi mcp` is the only sanctioned way captured content leaves the
    machine.** `search` and `transcript` refuse when the store needed a key to open, and `compress`
    refuses an encrypted store. It is a speed bump rather than a boundary, and `docs/encryption.md` is
    the honest statement of which. → `internal/cli/CLAUDE.md`, `internal/mcp/CLAUDE.md`
