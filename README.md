...
`search_events` returns both tracks of an audio chunk, each as its own row carrying its own `audio_source`. It does not merge them: a pair shares a 30-second interval, not necessarily a sound, and nothing a search result can see separates the microphone re-recording your speakers from a second conversation in the room. `get_transcript` is what answers that, per segment, against word timings and an energy envelope.
