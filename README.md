...
This is how you ask Lumi what happened. Lumi serves its index to MCP-capable agents — Claude Desktop, Claude Code, Cursor, Codex — and to Pi through a small extension. The agent brings its own model. Lumi does no inference.

Open **Settings → MCP** and press **Set up**. It configures every supported agent installed on this Mac, and running it twice changes nothing. A client that already has a *different* Lumi entry is never overwritten silently: it shows as a conflict, displays the entry that is in the way, and offers **Replace** for that one client. Clients Lumi does not know about get a **Copy client config** button with the exact snippet to paste, in whichever format that client reads.
